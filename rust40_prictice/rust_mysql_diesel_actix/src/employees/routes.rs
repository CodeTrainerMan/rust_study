...
async fn find_all()->Result<HttpResponse,CustomError>{
    let emplpyees = web::block(||Employees::find_all()).await.unwrap();
    Ok(HttpResponse::Ok().json(emplpyees))
}
...
async fn create(employee:web::Json<Employee>)->Result<HttpResponse,CustomError>{
    let emplpyee = Employees::create(employee.into_inner())?;
    Ok(HttpResponse::Ok().json(emplpyee))
}

#[put("/employees/{id}")]
async fn update(id:web::Path<i32>,employee:web::Json<Employee>)->Result<HttpResponse,CustomError>{
    let emplpyee = Employees::update(id.into_inner(),employee.into_inner())?;
    Ok(HttpResponse::Ok().json(emplpyee))
}
